...
- When on, shows the pre-written script for the selected topic — plain readable text the student can read aloud.
- Scripts stored in `src/data/topics.js` alongside topic metadata — one script per topic (18 total).
- Word-by-word live highlighting was built then removed — replaced with simple static display at user's direction.
...
### ✅ User Testing
- **`TEST_SCRIPT.md`** — 8-section, 35-case test script written for non-technical NGO testers.
- Covers: start screen, full conversation loop, feedback accuracy, voice behaviour, assisted mode, all 3 scenario modes, navigation edge cases.
- Includes qualitative feedback questions and a bug log table.
- Reformatted into 10 WhatsApp-ready plain text messages for easy distribution to test group.
